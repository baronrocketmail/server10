...
export default async function handler(req,res){
    ...
    if (req.method === "POST"){
        ...
        const fieldValue = await getInfo(paymentID)
...
async function getInfo(paymentID){
    const fieldValue = await getDoc(doc(db, "/units/18572 Cull Canyon Rd/payments", paymentID )).then(snapshot => snapshot.data())
